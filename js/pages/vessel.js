...
function specRows(v) {
  const rows = [
    ...
    ['Lunghezza x larghezza', v.loa !== null ? `${v.loa.toFixed(2)} x ${v.beam.toFixed(2)} m` : missing('DATO DA FORNIRE'), verified(v.verified.specs, v.source)],
    ['Ponte di carico', v.deckArea !== null ? `${v.deckArea} m2` : v.deckDims ? `${v.deckDims} m` : missing('DATO DA FORNIRE'), ''],
    ['Merci sul ponte', v.goodsT !== null ? `${v.goodsT} t` : missing('DATO DA FORNIRE'), ''],
    ['Acqua / carburante', v.waterMc !== null ? `${v.waterMc} m3 / ${v.fuelMc} m3` : missing('DATO DA FORNIRE'), ''],
