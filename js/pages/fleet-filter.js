...
function card(v) {
  ...
  const photo = v.photo
    ? `<img src="${v.photo}" alt="${v.name}, ${cls}" class="w-full h-full object-cover"/>`
  ...
  return `
    <article class="vessel-card bg-white border border-outline-variant rounded-lg overflow-hidden vessel-card-hover" data-vessel="${v.id}" data-reveal>
      ...
      <div class="p-5 flex flex-col gap-3">
        ...
        <dl class="grid grid-cols-3 gap-2 text-xs text-on-surface-variant">
          <div><dt class="uppercase tracking-wider text-[10px]">LOA</dt><dd>${v.loa !== null ? `${v.loaApprox ? 'circa ' : ''}${v.loa} m` : missing('m')}</dd></div>
          <div><dt class="uppercase tracking-wider text-[10px]">Anno</dt><dd>${cell(v.built, 'anno')}</dd></div>
