...
function roleItem(role) {
  ...
  const example = role.source !== 'live-site';
  return `
    <details class="career-role group border border-outline-variant rounded-lg bg-white open:shadow-md transition-shadow" data-area="${role.area}">
      <summary class="grid grid-cols-[1fr_auto] items-center gap-x-4 gap-y-2 p-5 cursor-pointer list-none [&::-webkit-details-marker]:hidden md:flex md:flex-wrap md:gap-x-6">
        <span class="md:flex-1 md:min-w-[14rem] font-headline-md text-lg text-primary">${esc(role.title_it)}</span>
        <span class="material-symbols-outlined text-primary transition-transform duration-300 group-open:rotate-180 md:order-last">expand_more</span>
        <span class="col-span-2 flex flex-wrap items-center gap-x-4 gap-y-2 md:contents">
          ...
          ${example ? '<span class="text-[11px] uppercase tracking-wider font-bold px-2.5 py-1 rounded-full border border-outline-variant text-on-surface-variant" title="Profilo esemplificativo: illustra un ruolo tipico della flotta, non una posizione attualmente pubblicata">Posizione Esemplificativa</span>' : ''}
        </span>
      </summary>
...
export function initCareers() {
  ...
  list.innerHTML = depts.map((dept) => `
    ...
    <p id="careers-empty" class="text-on-surface-variant text-sm py-10 text-center" hidden>Nessuna posizione aperta per questa combinazione di area e reparto.</p>`;

  const emptyMsg = document.getElementById('careers-empty');

  chips.innerHTML = AREAS.map(({ id, label }) => {
    const n = id === 'all' ? ROLES.length : ROLES.filter((r) => r.area === id).length;
    return `<button type="button" data-area="${id}" class="${CHIP} ${(id === 'all' ? CHIP_ON : CHIP_IDLE).join(' ')}">${label} <span class="font-semibold">${n}</span></button>`;
  }).join('');
